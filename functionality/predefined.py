...
def tablecreation():
    ...
    """
    #print("initial database data defined.")

    try:
        sqliteConnection = sqlite3.connect('test.db.sqlite3')
        conn = sqliteConnection.cursor()
        # print("Connected to SQLite")

        command = """CREATE TABLE IF NOT EXISTS 'test.db'(
            ...
            'streak_archive' INTEGER DEFAULT 0,
        )"""
    ...
    def predefinedhabits():
        ...
        conn.executemany(
            "INSERT OR REPLACE INTO habits ('Name', 'Description', 'Start_Date', 'Period_Count','Streak_Count','Streak_Archive') VALUES (?,?,?,?,?,?,?,?)",
            habits)
